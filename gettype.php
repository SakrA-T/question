<?php
	...
	if($type=="json"){
		...
		$json="[{\"schoolID\":\"school-0\",\"name\":\"综合\",\"type\":[";
		for($i=0;$i<$rows_pub;$i++){
			$row=mysqli_fetch_row($result_pub);
			$json.="{\"id\":\"type-".$row[0]."\",\"name\":\"".$row[1]."\"}";
			if($i!=$rows_pub-1)
				$json.=",";
		}
		$json.="]},";
		...
		$json.="{\"schoolID\":\"school-".$row[2]."\",\"name\":\"".$row[3]."\",\"type\":[";
		for($i=1;$i<$rows;$i++){
			if($row_[2]==$row[2])
				$json.="{\"id\":\"type-".$row[0]."\",\"name\":\"".$row[1]."\"},";
			else{
				$json.="{\"id\":\"type-".$row[0]."\",\"name\":\"".$row[1]."\"}";
				$json.="]},";
				$json.="{\"schoolID\":\"school-".$row_[2]."\",\"name\":\"".$row_[3]."\",\"type\":[";
			}
			$row=$row_;
			$row_=mysqli_fetch_row($result);
		}
